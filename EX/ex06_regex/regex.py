...
def find_words(text: str) -> list:
    ...
    """
    return re.findall(r'[A-ZÕÄÖÜ][a-zõäõü]+', text)
...
def find_words_with_vowels(text: str) -> list:
    ...
    """
    return re.findall(r'[AEIOUÖÄÜÕ][a-zöäüõ]+', text)
...
def find_words_from_sentence(sentence: str) -> list:
    ...
    """
    return re.findall(r'[A-ZÄÖÜÕa-zäöüõ0-9]+', sentence)
...
def find_years(text: str) -> list:
    ...
    """
    return [int(match) for match in re.findall(r'(?<![0-9])[0-9]{4}(?![0-9])', text)]
...
def find_phone_numbers(text: str) -> dict:
    ...
    """
    dic = {}
    numbers = re.findall(r'((\+[0-9]{3})\s?)?([0-9]{7,8})', text)
    for i in numbers:
        area = i[1]
        number = i[2]
        if area not in dic:
            dic[area] = list()
            dic[area].append(number)
        elif area in dic:
            dic[area].append(number)
    return dic
