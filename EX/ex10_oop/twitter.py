...
def find_fastest_growing(tweets: list[Tweet]) -> Tweet:
    """
...
def sort_by_popularity(tweets: list[Tweet]) -> list:
    """
...
def filter_by_hashtag(tweets: list[Tweet], hashtag: str) -> list:
    """
...
def sort_hashtags_by_popularity(tweets: list[Tweet]) -> list:
    """
    ...
    sum_dic = {}
    for tweet in tweets:
        for hashtag in re.findall(r'#[A-Za-z]+', tweet.content):
    ...
    for hashtag in hashtag_list:
        ...
        sum_dic[hashtag] = tweet_retweets
        tweet_retweets = 0
    return list(dict(sorted(sum_dic.items(), key=lambda item: item[1], reverse=True)).keys())
